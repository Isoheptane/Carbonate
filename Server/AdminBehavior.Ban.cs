...
namespace Carbonate.Server
{
    ...
    {   
        ...
        void AdminBan(OnlineUser user, CommandPacket command)
        {
            ...
            Broadcast("server", $"\\cr{target.nickname}\\rr({target.username}\\rr)\\cr is banned until {untilTime.ToString("yyyy/MM/dd HH:mm:ss")}.");
        ...
        void AdminUnban(OnlineUser user, CommandPacket command)
        {
            ...
            {
                ServerMessage("server", user, $"\\cr{target.nickname}\\rr({target.username}\\rr)\\cr is not banned now.");
                return;
            }

            Users[arguments[0]].banTime = DateTime.MinValue;

            Broadcast("server", $"\\ar{target.nickname}\\rr({target.username}\\rr)\\ar is unbanned.");
        }
    }
}